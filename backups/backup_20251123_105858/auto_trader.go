...
func (at *AutoTrader) executeCloseLongWithRecord(decision *decision.Decision, actionRecord *logger.DecisionAction) error {
	...
	// 🔧 BUG FIX: 从订单返回信息中获取实际成交价格
	if avgPrice, ok := order["avgPrice"].(string); ok && avgPrice != "" {
		// 币安返回的avgPrice是字符串，需要转换
		if price, err := strconv.ParseFloat(avgPrice, 64); err == nil {
			actionRecord.Price = price
			log.Printf("  ✓ 平仓成功，实际成交价: %.4f", price)
		} else {
			// 如果解析失败，使用市场价格作为fallback
			actionRecord.Price = marketData.CurrentPrice
			log.Printf("  ✓ 平仓成功，使用市场价: %.4f", marketData.CurrentPrice)
		}
	} else if price, ok := order["price"].(float64); ok && price > 0 {
		// 某些交易所直接返回float64类型的price
		actionRecord.Price = price
		log.Printf("  ✓ 平仓成功，实际成交价: %.4f", price)
	} else {
		// 如果订单信息中没有价格，使用市场价格
...
func (at *AutoTrader) executePartialCloseWithRecord(decision *decision.Decision, actionRecord *logger.DecisionAction) error {
	...
	if avgPrice, ok := order["avgPrice"].(string); ok && avgPrice != "" {
		...
		if price, err := strconv.ParseFloat(avgPrice, 64); err == nil {
			actionRecord.Price = price
			log.Printf("  ✓ 部分平仓成功: 平仓 %.4f (%.1f%%), 剩余 %.4f, 成交价: %.4f",
				closeQuantity, decision.ClosePercentage, remainingQuantity, price)
		} else {
