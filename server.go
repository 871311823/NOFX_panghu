...
func (s *Server) analyzePerformanceFromBinance(traderInstance trader.Trader, lookbackDays int) (*logger.PerformanceAnalysis, error) {
	...
	if len(analysis.RecentTrades) >= 2 {
		...
		for _, trade := range analysis.RecentTrades {
			// 收益率 = 盈亏 / 开仓价值（估算）
			positionValue := trade.OpenPrice * 100 // 假设每次交易100单位
			if positionValue > 0 {
				returnRate := trade.PnL / positionValue
		...
		if len(returns) >= 2 {
			...
			if stdDev > 0 {
				...
				// 年化夏普比率（假设每天1笔交易）
				analysis.SharpeRatio *= math.Sqrt(365)
				
				// 限制范围 [-10, 10]，避免异常值
				if analysis.SharpeRatio > 10 {
					analysis.SharpeRatio = 10
				} else if analysis.SharpeRatio < -10 {
					analysis.SharpeRatio = -10
				}
			}
		}
